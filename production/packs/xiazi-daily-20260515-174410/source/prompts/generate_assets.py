...
BG_TOP = (255, 238, 193, 255)
BG_BOTTOM = (183, 226, 202, 255)
TEAL = (81, 167, 151, 255)
...
def make_icon() -> Image.Image:
    ...
    if not bbox:
        raise RuntimeError("Approved master has empty alpha")
    left, top, right, bottom = bbox
    h = bottom - top
    # Keep a little neck/shoulder under the face so the 50px icon does not end exactly at the chin.
    head = master.crop((left, top, right, top + round(h * 0.66)))
    crop = crop_alpha(head, padding=2)
    fitted = contain(crop, 44, 44)
    canvas = Image.new("RGBA", (50, 50), (0, 0, 0, 0))
    canvas.alpha_composite(fitted, ((50 - fitted.width) // 2, 2))
    return canvas
...
def make_banner(source_crops: list[Image.Image]) -> Image.Image:
    ...
    # Coherent soft landscape base; decorative shapes stay in the background only.
    draw.polygon(
        [(0, 282), (70, 266), (165, 276), (254, 252), (350, 270), (452, 244), (560, 265), (650, 250), (750, 270), (750, 400), (0, 400)],
        fill=(111, 190, 164, 255),
    )
    draw.polygon(
        [(0, 322), (92, 304), (205, 318), (324, 292), (442, 315), (560, 294), (678, 310), (750, 296), (750, 400), (0, 400)],
        fill=(82, 169, 149, 255),
    )

    # A subtle shared path ties the four stickers into one daily-life scene.
    path_points = [(88, 250), (210, 214), (340, 234), (482, 202), (626, 228)]
    draw.line(path_points, fill=(255, 246, 210, 150), width=12, joint="curve")
    draw.line(path_points, fill=(247, 184, 96, 90), width=3, joint="curve")

    for x, y, r, color in [
        (70, 130, 7, (255, 246, 196, 255)),
        (178, 154, 6, (255, 226, 146, 255)),
        (106, 282, 12, YELLOW),
        (238, 244, 9, PEACH),
        (396, 226, 10, PINK),
        (514, 218, 7, (255, 246, 196, 255)),
        (602, 246, 11, YELLOW),
        (674, 278, 8, PEACH),
    ]:
        draw.ellipse((x - r, y - r, x + r, y + r), fill=color)

    paste_fit(bg, source_crops[0], (36, 56, 210, 334))
    paste_fit(bg, source_crops[5], (222, 42, 386, 310))
    paste_fit(bg, source_crops[11], (402, 52, 570, 336))
    paste_fit(bg, source_crops[16], (554, 44, 728, 332))
...
def make_review_images(source_crops: list[Image.Image], stickers: list[Image.Image], cover: Image.Image, icon: Image.Image, banner: Image.Image, items: list[StickerItem]) -> None:
    ...
    draw.text((34, 28), "Xiazi identity review", font=font(34), fill=(70, 48, 34))
    ...
    draw.text((34, 24), "Xiazi WeChat export review", font=font(30), fill=(30, 34, 40))
...
def validate(spec: dict[str, Any], sticker_paths: list[Path], cover_path: Path, icon_path: Path, banner_path: Path) -> dict[str, Any]:
    ...
    add("source-linkage-generation-record-names-approved-master", "master/approved/xiazi-approved-master.png" in record_text, "generation-record.md contains approved master path")
...
def write_metadata_form_reports(spec: dict[str, Any], items: list[StickerItem], validation: dict[str, Any]) -> None:
    ...
    metadata = {
        ...
        "sourceLinkage": {
            ...
            "sourceReference": "source/references/cx-reference.png",
            "masterInput": "master/input/cx-master-reference.png",
            "identityPreservationMethod": "raw sticker sheets generated by image model after the approved master image was loaded as visual reference; local script only processed model outputs",
        },
        "assets": {
            "stickers": stickers,
            "cover": "cover.png",
...
def append_generation_record() -> None:
    marker = "## Raw Sticker Sheets 2026-05-15"
